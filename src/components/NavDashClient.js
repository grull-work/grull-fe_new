...
export default function DashboardLayout({ children, section }) {
  ...
  const handleShareProfile = async()=>{
    ...
    // console.log("clicked")
    const url=`https://grull.work/client/profile/${userId}`;
   try {
  ...
  const handleButtonClick = btn=>{
    const map={home:'/client',wallet:'/client/wallet',manageJobs:'/clientmanagejobs/posted'};
    navigate(map[btn]);
    setMobileOpen(false);
  };
  ...
  const avatarBackgroundColor = fullname?stringToColor(fullname):'#666';
    ...
    <Box sx={{ display:'flex' }}>
      ...
      <AppBar position="fixed"
        ...
        <Toolbar sx={{ bgcolor:'#EDEDED', boxShadow:'0 0 4px rgba(0,0,0,0.25)' }}>
          ...
          <IconButton onClick={()=>navigate('/clientchat')} sx={{ textTransform:'none', mr:{xs:"0" ,sm:"2"}, color :'rgb(0, 0, 0)', display:{ xs:'none',sm:"flex"} }}>
            <FiMessageSquare size={24}/>
          </IconButton>
          <IconButton onClick={handleNotificationIconClick} sx={{ color:'rgb(65, 65, 65)', mr:isDesktop?2:0 }}>
            <IoNotificationsOutline size={24}/>
          </IconButton>
          {/* {!isDesktop && ( */}
            <Box ref={containerRef} sx={{ position:'relative', ml:2 }}>
              {profileImage
